...
lines = [l.strip() for l in raw_csv.strip().split("\n") if l.strip() and not l.startswith("SBM 2026") and not l.startswith(";;;")]
...
print(f"Total provinces parsed: {len(sbm_map)}")
for k, v in sbm_map.items():
    print(f"  {k}: Sewa Minibus Bulanan = Rp {v['sewa_minibus_bulan']:,} | Sewa Harian = Rp {v['sewa_harian']:,} | Tiket PP = Rp {v['tiket_pesawat']:,} | Makan Rapat = Rp {v['makan_rapat']:,}")

with open("scratch/sbm_parsed_map.json", "w", encoding="utf-8") as f:
    json.dump(sbm_map, f, indent=2)
